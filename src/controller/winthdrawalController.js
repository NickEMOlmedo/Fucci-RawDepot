...
export const updateWithdrawal = async (req, res) => {
  try {
    ...
    if (error.code === 'P2025') {
      return res.status(400).json({ error: 'Retiro no encontrado!' })
    } else {
...
export const deleteWithdrawal = async (req, res) => {
  try {
    ...
    if (error.code === 'P2025') {
      return res.status(400).send({ error: 'Retiro no encontrado!' })
    } else {
      return res.status(500).json({
...
export const searchWithdrawalWithDate = async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Error al buscar los retiros.' })
  } finally {
    prisma.$disconnect()
  }
}
